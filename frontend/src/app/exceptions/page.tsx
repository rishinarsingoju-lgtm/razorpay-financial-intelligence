...
import { getExceptions, getTransactions, updateExceptionStatus, type ExceptionRecord } from "@/lib/api";
...
export default function ExceptionsPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    Promise.all([getExceptions({ type, severity, status }, controller.signal), getTransactions(controller.signal)])
      .then(([nextExceptions, transactions]) => {
        setExceptions(nextExceptions);
        setPaymentReferences(Object.fromEntries(transactions.map((transaction) => [transaction.id, transaction.razorpay_payment_id])));
      })
